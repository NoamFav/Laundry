...
export default function WashingMachine({ status, program, onStart, onStop, currentUser, queue = [] }) {
      ...
      >
        ...
        <div className="bg-gray-900 rounded-xl p-3 mb-4">
          <div className="text-green-400 font-mono text-xs mb-1">
            {status === 'idle' && 'READY'}
            {status === 'running' && `WASHING${'.'.repeat((rotation / 120) % 4)}`}
            {status === 'done' && 'COMPLETE!'}
          </div>
          {program && (
            <div className="text-white text-sm font-semibold">{program.name}</div>
          )}
          {currentUser && (
            <div className="text-gray-400 text-xs mt-1">User: {currentUser}</div>
          )}
        </div>
