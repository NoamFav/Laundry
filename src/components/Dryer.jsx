...
export default function Dryer({ status, program, onStart, onStop, currentUser, queue = [] }) {
      ...
      >
        ...
        <div className="bg-gray-900 rounded-xl p-3 mb-4">
          <div className="text-orange-400 font-mono text-xs mb-1">
            {status === 'idle' && 'READY'}
            {status === 'running' && `DRYING${'.'.repeat((rotation / 120) % 4)}`}
          ...
          {status === 'running' && (
            <div className="flex items-center gap-1 mt-2">
              ...
              <div className="text-xs text-gray-400">Heat Active</div>
            </div>
          )}
        </div>
