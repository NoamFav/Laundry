...
export default function LaundryRoom() {
  ...
  const startMachine = async (machineType, program) => {
    const machinePath = machineType === 'washer' ? 'laundry/washingMachine' : 'laundry/dryer';
    ...
    await set(ref(database, machinePath), {
      ...
      queue: machineType === 'washer' ? (washingMachine?.queue || []) : (dryer?.queue || [])
    });
  ...
  const stopMachine = async (machineType) => {
    ...
    if (machineData.status === 'running') {
      ...
      if (queue.length > 0) {
        ...
        const program = (machineType === 'washer' ? WASHING_PROGRAMS : DRYER_PROGRAMS).find(
          (p) => p.id === nextItem.programId
        );
  ...
  const addToQueue = async (machineType, program) => {
    ...
    await set(ref(database, `${machinePath}/queue`), [...currentQueue, queueItem]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-4 pb-24">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="text-center py-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Laundry Room</h1>
          <p className="text-gray-600">Ground Floor</p>
        </div>

        {/* Washing Machine */}
        <div>
          <h2 className="text-xl font-semibold text-gray-700 mb-3">Washing Machine</h2>
          <WashingMachine
        ...
        {/* Dryer */}
        <div>
          <h2 className="text-xl font-semibold text-gray-700 mb-3">Dryer</h2>
          <Dryer
            status={dryer.status}
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                {(showProgramModal === 'washer' ? WASHING_PROGRAMS : DRYER_PROGRAMS).map(
                  (program) => {
                    const machineData = showProgramModal === 'washer' ? washingMachine : dryer;
                    const isIdle = machineData.status === 'idle';
                    ...
                    return (
                      <motion.button
                        ...
                        onClick={() =>
                          isIdle
                            ? startMachine(showProgramModal, program)
                            : addToQueue(showProgramModal, program)
                        }
              ...
              {(showProgramModal === 'washer' ? washingMachine : dryer).status !== 'idle' && (
                <div className="mt-4 text-sm text-gray-600 text-center">
                  Machine is busy. Program will be added to queue.
                </div>
              )}
            </motion.div>
          </motion.div>
