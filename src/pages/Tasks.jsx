...
import { ChefHat, Droplets, FileText, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-4 pb-24">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center py-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">House Tasks</h1>
          <p className="text-gray-600">Manage shared responsibilities</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-semibold text-gray-800">Kitchens</h2>
          </div>

          {Object.entries(tasks.kitchens).map(([kitchenId, kitchenTasks]) => {
            const facilityConfig = FACILITIES.kitchens[kitchenId];
            return (
              <div key={kitchenId} className="mb-6 last:mb-0">
                ...
                <div className="space-y-2">
                  {Object.entries(kitchenTasks).map(([taskName, taskData]) => {
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          {isMyTaskFlag && (
                            <motion.button
                              ...
                              whileTap={{ scale: 0.95 }}
                              onClick={() => completeKitchenTask(kitchenId, taskName)}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-semibold text-gray-800">Showers</h2>
          </div>

          {Object.entries(tasks.showers).map(([showerId, showerData]) => {
            const facilityConfig = FACILITIES.showers[showerId];
            const isMyTaskFlag = isMyTask(showerData.currentRoom);
            return (
              <motion.div
                key={showerId}
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      Current: {showerData.currentRoom}
                      {isMyTaskFlag && (
                        <span className="ml-2 text-purple-600 font-semibold">
                          (Your turn!)
                        </span>
                  ...
                  {isMyTaskFlag && (
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => completeShowerTask(showerId)}
                      className="bg-green-500 text-white px-4 py-2 rounded-lg font-semibold shadow-lg flex items-center gap-2"
                    >
                      <CheckCircle className="w-4 h-4" />
                      Done
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-semibold text-gray-800">Toilets (Paper Status)</h2>
          </div>

          {Object.entries(tasks.toilets).map(([toiletId, toiletData]) => {
            const facilityConfig = FACILITIES.toilets[toiletId];
            const isMyTaskFlag = toiletData.currentRoom && isMyTask(toiletData.currentRoom);
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      {toiletData.currentRoom && (
                        <div className="text-sm text-gray-600">
                          Next refill: {toiletData.currentRoom}
                          {isMyTaskFlag && (
                            <span className="ml-2 text-purple-600 font-semibold">
                              (Your turn!)
                            </span>
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-white text-xs font-semibold ${
                        paperStatusColor[toiletData.paperStatus]
                      }`}
                    >
                  ...
                  <div className="flex gap-2">
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => updateToiletPaper(toiletId, 'full')}
                    ...
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => updateToiletPaper(toiletId, 'low')}
                    ...
                    <motion.button
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => updateToiletPaper(toiletId, 'empty')}
